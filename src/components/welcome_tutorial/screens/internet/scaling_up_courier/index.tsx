...
const RoutingTable: React.FC<PropsWithChildren> = ({ children }) => {
  return (
    <Table>
      <tr>
...
export const ScalingUp: React.FC = () => {
  const renderTableRow = (codeRange: string, next: string, cost: number) => (
    <tr>
      <td>{codeRange}</td>
      <td>{next}</td>
      <td>{cost}</td>
  ...
  return (
    <div id={styles.container}>
      ...
      <Table>
        ...
        {[
          ["606xx", "Chicago, IL", "Chicago Regional Hub, W Madison St, 60661"],
      ...
      <p>
        ...
        deliveries of shipments and{" "}
        <b>will serve the most specific zip codes</b>. We will have{" "}
        <b>one office per zip code</b>.
        <br />
      ...
      <p>
        Regional hubs cover a broader geographical area, i.e.{" "}
        <b>a range of zip codes</b>. We will use{" "}
      ...
      <h4 className={commonStyles.sub_heading}>
        Storing the Routes: Structure
      </h4>
      <p>
        With a hierarchical structure in place, we pave the way for implementing
        a prefix-based routing logic for each of our hubs using route
        aggregation.
      </p>
      <ul className={commonStyles.list}>
        <li>
          Each hub stores a {italicBold("Routing table")} for storing the
          routes.
        </li>
        <li>
          This table is built using prefix-aggregation of zip codes and
          pre-existing knowledge about hubs serving the zip codes.
        </li>
        <li>It has the following columns:</li>
        <ul>
          <li>
            {italicBold("Zip Code Range")} specifying the zip code lookup.
          </li>
          <li>
            {italicBold("Next Office")} specifying the next office to route a
            shipment to, depending on the zip code range.
          </li>
          <li>
            {italicBold("Estimated Time to Deliver")} specifies the estimated
            time to deliver to this office{" "}
          </li>
        </ul>
        <p>
          This structure allows us to ensure that only the local hubs store and
          route to individual addresses. The regional and national hubs only
          need to store the routes to other hubs.
        </p>
      </ul>
      <h4 className={commonStyles.sub_heading}>
      ...
      <ul className={commonStyles.list}>
        <li>
          ...
          itself. For any other zip, it routes packages to the Chicago Hub.
          <RoutingTable>
            {renderTableRow("60661", "-", 0)}
            {renderTableRow("xxxxx", "Chicago Regional Hub", 5)}
          </RoutingTable>
        </li>
        <li>
          {underlineBold("Chicago, 60666")}: It handles deliveries to 60666
          itself. For any other zip, it routes packages to the Chicago Hub.
          <RoutingTable>
            {renderTableRow("60666", "-", 0)}
            {renderTableRow("xxxxx", "Chicago Regional Hub", 5)}
          </RoutingTable>
        </li>
        <li>
          {underlineBold("Detroit, 48127")}: It handles deliveries to 48127
          itself. For any other zip, it routes packages to the Detroit Hub.
          <RoutingTable>
            {renderTableRow("48127", "-", 0)}
            {renderTableRow("xxxxx", "Detroit Regional Hub", 5)}
          </RoutingTable>
        </li>
        <li>
          {underlineBold("Detroit, 48201")}: It handles deliveries to 48201
          itself. For any other zip, it routes packages to the Detroit Hub.
          <RoutingTable>
            {renderTableRow("48201", "-", 0)}
            {renderTableRow("xxxxx", "Detroit Regional Hub", 5)}
          </RoutingTable>
        </li>
        <li>
          ...
          <RoutingTable>
            {renderTableRow("60661", "Chicago 60661 Last Mile Hub", 5)}
            {renderTableRow("60666", "Chicago 60666 Last Mile Hub", 5)}
            {renderTableRow("xxxxx", "Midwest Regional Hub", 10)}
          </RoutingTable>
        </li>
        <li>
          {underlineBold("Detroit Hub, MI")}: It handles deliveries to the
          Detroit area and routes other packages to the Midwest National Hub.
          <RoutingTable>
            {renderTableRow("48127", "Detroit 48127 Last Mile Hub", 15)}
            {renderTableRow("48201", "Detroit 48127 Last Mile Hub", 15)}
            {renderTableRow("xxxxx", "Midwest Regional Hub", 10)}
          </RoutingTable>
        </li>
        <li>
          ...
          belonging to Northeast, it routes the packages to the Northeast
          National Hub.
          <RoutingTable>
            {renderTableRow("606xx", "Chicago Regional Hub", 10)}
            {renderTableRow("481xx", "Detroit Regional Hub", 10)}
            {renderTableRow("482xx", "Detroit Regional Hub", 10)}
            {renderTableRow("0xxxx", "Northeast National Hub", 24)}
            {renderTableRow("1xxxx", "Northeast National Hub", 24)}
            {renderTableRow("xxxxx", "U.S. Central National Hub", 48)}
          </RoutingTable>
        </li>
      </ul>
      <h4 className={commonStyles.sub_heading}>
        {Emoji.Bulb} Try to come up with the routing tables of all the other
        hubs.
      </h4>
